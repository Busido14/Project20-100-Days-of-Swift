...
//  Project20
//
//  Created by Артем Чжен on 08/05/23.
//
...
class GameScene: SKScene {
    ...
    var score = 0 {
        didSet {
            gameScoreLabel.text = "You score: \(score)"
        }
    }
    override func didMove(to view: SKView) {
        ...
        gameScoreLabel.position = CGPoint(x: 512, y: 700)
    ...
    func explode(firework: SKNode) {
        if let emitter = SKEmitterNode(fileNamed: "explode") {
            ...
            let time = SKAction.wait(forDuration: 1)
            let remove = SKAction.run {
                emitter.removeFromParent()
            }
            let action = SKAction.sequence([time, remove])
            emitter.run(action)
        }
        firework.removeFromParent()
    }
